...
public class PenguinAgent : Agent
{
    ...
    [Tooltip("How fast the agent moves turns")]
    ...
    public override void AgentAction(float[] vectorAction)
    {
        ...
        //Apply a tiny negative reward every step to encouage
    ...
    /// </summary>
    /// <returns>A vectorAction array of floats that will be passed in to<see cref="="AgentAction(float[])"/></returns>
    ...
    {
        //Whether the penguin has eaten a fish ( 1 float
        AddVectorObs(isFull);

        //Distance to the baby (1float = 1 value)
        AddVectorObs(Vector3.Distance(baby.transform.position, transform.position));

        // Direction to baby (1 Vector3 = 3values)
        AddVectorObs((baby.transform.position - transform.position).normalized);

        // Direction penguin is facing (1 Vector3 = 3values)
